...
if [ $fileProcessUtiltyStatusCode -eq 200 ];
		then
			...
			curl -X POST "$fileCpyAPIURL" -H "accept: */*" -H "Content-Type: application/json" -d "{\"destination\":\"[{ \"destFilePath\": \"$fileDownloadPrcoessPath\", \"destServerName\": \"$destServerName\"}]\",\"serverName\": \"$serverName\", \"sourceFileName\": \"DeviceDatabase.jsonl_ProcessedFile\", \"sourceFilePath\": \"\$fileDownloadPrcoessPath"}""
